...
resource "aws_subnet" "tf_public_subnet" {
    ...
    tags = {
        Name = join("-",[var.public_subnet_names,"${count.index}" , tostring(data.aws_availability_zones.vpc_az.names[count.index%length(data.aws_availability_zones.vpc_az.names)])])
        ManagedBy = "${var.controller_name}"
  }
}
...
resource "aws_subnet" "tf_private_subnet" {
    ...
    tags = {
        Name = join("-", [var.private_subnet_name ,"${count.index}", tostring(data.aws_availability_zones.vpc_az.names[count.index%length(data.aws_availability_zones.vpc_az.names)])])
        ManagedBy = "${var.controller_name}"
  }
}
